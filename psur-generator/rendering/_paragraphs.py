...
class ParagraphMixin:
    ...
    def _build_chart_context(self, psur: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        top_mdp = max(rates_by_imdrf, key=lambda r: field(r, "complaint_count", 0), default={})
        mdp_note = (
            f"Interpretation: the leading medical device problem was {field(top_mdp, 'category')} "
            f"with {int(field(top_mdp, 'complaint_count', 0))} complaints "
            f"({num(field(top_mdp, 'percentage')):.4f}% of distributed units), which is the primary driver "
