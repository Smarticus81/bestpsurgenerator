...
# Knowledge layer: precision retrieval of regulatory rules per section.
# Gated by env var KB_LEGACY_PROMPTS=1 (falls back to legacy embedded blocks).
try:  # pragma: no cover - optional dependency at import time
    from knowledge import (
        get_registry as _kb_get_registry,
        Query as _KBQuery,
        retrieve as _kb_retrieve,
        render_rules_for_prompt as _kb_render,
    )
    _KB_AVAILABLE = True
...
_DEPTH_ENHANCEMENT_ENABLED = (
    os.getenv("PSUR_DEPTH_ENHANCEMENT", "0").lower() in {"1", "true", "yes"}
)
_TABLE_COMPLETION_ENABLED = (
    os.getenv("PSUR_TABLE_COMPLETION", "0").lower() in {"1", "true", "yes"}
)
...
class SectionAgent:
    ...
    def __init__(self, section_key: str, global_context: str = "", uk_market_detected: bool = False,
        ...
        # Knowledge-layer state (populated each generate() call)
        self._kb_statistics: Dict[str, Any] = {}
        self._kb_device_context: Dict[str, Any] = {}
        self._kb_retrieved_rule_ids: List[str] = []
    ...
    def _execute_search_parsed_data(self, query: str, parsed_data: Optional[Dict[str, Any]]) -> str:
        """Search the raw parsed data for a specific query."""
        if not parsed_data:
            return "No raw parsed data available to search."
        ...
        def search_dict(d, current_path=""):
            for k, v in d.items():
                ...
                if isinstance(v, dict):
                    ...
                    results.append(f"{path}: {v}")
                    
        search_dict(parsed_data)
        ...
        if len(results) > 50:
            res_str += f"\n... and {len(results) - 50} more omitted."
    ...
    def generate(
        ...
        # Capture inputs for knowledge-layer retrieval inside _build_system_prompt
        self._kb_statistics = statistics or {}
        self._kb_device_context = device_context or {}
        ...
        # Expose a simple data query tool for SOTA active tool calling
        tools = [
            {
                "name": "search_parsed_data",
                "description": "Search the raw parsed data for specific terms or context if the pre-calculated statistics are insufficient. Use this if you need deeper context.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "The search term or key to look for in the raw data."
                        }
                    },
                    "required": ["query"]
                }
        ...
        for attempt in range(3):  # initial + 2 retries
            tool_iterations = 0
            while tool_iterations < 5:  # Max 5 tool calls per attempt to prevent infinite loops
                t0 = time.time()
                ...
                if tool_calls:
                    ...
                    # Record the assistant's tool use message
                    assistant_content = []
                    # Keep any text the assistant generated alongside the tool call
                    if response.content and response.content[0].text:
                        assistant_content.append({"type": "text", "text": response.content[0].text})
                        
                    for tc in tool_calls:
                        print(f"  [{self.section_key}] Executing tool: {tc.name}({tc.input})", file=sys.stderr)
                    ...
                    # Execute the tools and provide results
                    user_content = []
                    for tc in tool_calls:
                        if tc.name == "search_parsed_data":
                            query = tc.input.get("query", "")
                            result_str = self._execute_search_parsed_data(query, parsed_data)
                        else:
                    ...
                    continue  # Loop back to let the LLM continue processing
                    
                # No tool calls, break out of tool loop
                break

            # If we exited the tool loop and just have text, parse it
            ...
            try:
                ...
                import sys
                print(
                    f"  [{self.section_key}] JSON parse failed on attempt {attempt+1}: {jde.msg}",
                    file=sys.stderr,
                )
                # Treat as a schema error so the retry loop fires with explicit JSON-format guidance.
                if attempt == 2:
                    # Last attempt — return best previous parsed result if any, else minimal stub
                    if best_result is not None:
                        return self._attach_kb_meta(best_result)
                    # Build a minimal valid stub from the schema's required keys
                    stub = {"narrative": content[:2000] if content else "Section generation failed — see logs."}
                    return self._attach_kb_meta(stub)
                messages = [
                    {"role": "user", "content": user_prompt},
                    {"role": "assistant", "content": content or "(empty response)"},
                    {"role": "user", "content": (
                        f"Your previous response was NOT valid JSON. The parser reported: {jde.msg}.\n\n"
                        f"Output ONLY a single JSON object for Section {self.section_key}. "
...
- Output ONLY valid JSON, no explanation."""
    ...
    def remediate(
        ...
        messages = [{"role": "user", "content": user_prompt}]
        best_remediated = section_content
        best_errors = None
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if best_errors is None or len(schema_errors) < len(best_errors):
                    best_remediated = remediated
                    best_errors = schema_errors

                if attempt == 2:
                    print(
                        f"  [{self.section_key}] Remediation produced {len(best_errors)} "
                        f"schema errors after 3 attempts — keeping original",
                        file=sys.stderr,
                    )
                    return section_content

                # Debate loop: add assistant's response and the auditor's critique
                messages.append({"role": "assistant", "content": content})
                error_list = "\n".join(f"- {e}" for e in schema_errors[:20])
                messages.append({
                    "role": "user",
                    "content": f"The auditor reviewed your output and found {len(schema_errors)} schema errors:\n{error_list}\nPlease fix these issues and output the corrected JSON."
                })

            except Exception as e:
                print(
                    f"  [{self.section_key}] Remediation failed on attempt {attempt+1}: {e}",
                    file=sys.stderr,
                )
                if attempt == 2:
                    return section_content
...
- Output ONLY valid JSON, no explanation."""
    ...
    def _build_kb_findings(self) -> Dict[str, Any]:
        """Derive symbolic finding flags consumed by rule trigger expressions."""
        stats = self._kb_statistics or {}
        trend = stats.get("trend_analysis") or {}
        ract_breaches = stats.get("ract_breaches") or []
        return {
    ...
    def _build_kb_rules_block(self) -> str:
        """Return a precision-retrieval block of rules for this section.

        Returns "" if knowledge layer unavailable or KB_LEGACY_PROMPTS=1.
        Side effect: stores the retrieved rule IDs on ``self`` for later
        attachment to the section's ``_meta.rules_applied`` array.
        """
        self._kb_retrieved_rule_ids = []
        if not _KB_AVAILABLE:
            return ""
        if os.environ.get("KB_LEGACY_PROMPTS") == "1":
            return ""
        try:
            registry = _kb_get_registry()
            dc = self._kb_device_context or {}
            markets = set()
            if self.uk_market_detected:
                markets.add("UK")
            # Always include EU as default market when not Class-I-only context
            markets.add("EU")
            query = _KBQuery(
                section=self.section_key,
                device_class_eu=dc.get("device_class_eu"),
                device_class_uk=dc.get("uk_mdr_classification_and_rule"),
                sterility=dc.get("sterility_status"),
                markets=markets,
                findings=self._build_kb_findings(),
                max_rules=int(os.environ.get("KB_MAX_RULES", "30")),
            )
            scored = _kb_retrieve(query, registry)
            self._kb_retrieved_rule_ids = [sr.rule.id for sr in scored]
            return _kb_render(scored)
        except Exception as exc:  # pragma: no cover
            import sys
            print(f"  [{self.section_key}] KB retrieval failed: {exc}", file=sys.stderr)
            return ""

    def _attach_kb_meta(self, parsed: Dict[str, Any]) -> Dict[str, Any]:
        """Attach knowledge-layer provenance under the section's ``_meta`` key.

        Preserves any rule IDs the agent self-cited under ``_meta.rules_applied``
        and merges them with the IDs the registry retrieved for this section.
        """
        if not isinstance(parsed, dict):
            return parsed
        meta = parsed.get("_meta") if isinstance(parsed.get("_meta"), dict) else {}
        agent_cited = []
        for entry in meta.get("rules_applied") or []:
            if isinstance(entry, str):
                agent_cited.append(entry)
            elif isinstance(entry, dict) and entry.get("id"):
                agent_cited.append(entry["id"])
        merged_ids = list(dict.fromkeys(agent_cited + (self._kb_retrieved_rule_ids or [])))
        meta["rules_applied"] = merged_ids
        meta["rules_retrieved_count"] = len(self._kb_retrieved_rule_ids or [])
        meta["section_key"] = self.section_key
        try:
            from knowledge import KNOWLEDGE_VERSION
            meta["knowledge_version"] = KNOWLEDGE_VERSION
        except Exception:
            pass
        parsed["_meta"] = meta
        return parsed
    ...
    def _build_system_prompt(self) -> List[Dict[str, Any]]:
        ...
        # Block 1: Global persistent context (shared across all sections)
        global_text = f"{self.global_context}\n\n{build_section_constraints()}\n\n{build_fabrication_block()}"

        # Block 2: Section-specific instructions
        section_text = f"""{'=' * 60}
...
{self._build_mdcg_regulatory_context()}"""
        ...
        # Block: Knowledge-layer retrieval (precision rules pulled from registry).
        # Disable by setting KB_LEGACY_PROMPTS=1.
        kb_block = self._build_kb_rules_block()
        if kb_block:
            section_text += "\n\n" + kb_block
