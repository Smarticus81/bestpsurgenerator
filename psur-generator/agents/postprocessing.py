...
def _build_allowed_identifier_set(parsed_data: Dict[str, Any]) -> set:
    ...
    for fsca in fsca_rows or []:
        if isinstance(fsca, dict):
            for key in ("fsca_id", "reference_number", "action_id", "id"):
                v = fsca.get(key)
                if v:
                    allowed.add(str(v).strip().upper().replace(" ", "").replace("-", ""))
    return allowed
...
_TEMPLATE_BRACKET_PATTERNS = [
    ...
    re.compile(r"\[\s*Delete this row if not applicable[^\]]*\]", re.IGNORECASE),
    re.compile(r"\[\s*Delete (?:this )?row[^\]]*\]", re.IGNORECASE),
    ...
    re.compile(r"\(\s*Delete this row if not applicable\s*\)", re.IGNORECASE),
    re.compile(r"\(\s*see (?:Annex|Appendix)[^)]*\)", re.IGNORECASE),
    # Bare prose debris emitted when the LLM echoes a template hint.
    re.compile(r"\bfor complete listing\b\.?", re.IGNORECASE),
    re.compile(r"\bDelete this row if not applicable\b\.?", re.IGNORECASE),
    re.compile(r"\bComplete listing\s+(?:in|at)\s+Appendix[^.]*\bfor complete listing\b\.?", re.IGNORECASE),
]
...
def _strip_template_debris_string(value: str) -> str:
    ...
    for pat in _SEE_DOC_PATTERNS:
        result = pat.sub("", result)
    # Collapse template-fragment stutters introduced when the LLM glues a
    # filled value next to its own template placeholder. Patterns observed:
    #   "a prior a prior FSCA"          -> "a prior FSCA"
    #   "a a CAPA from … the previous reporting period the previous reporting period-001"
    #   "the the previous reporting"    -> "the previous reporting"
    # Rule: collapse any run of 2-6 words immediately repeated, ignoring
    # leading determiners.
    result = re.sub(
        r"\b(a|an|the)\s+\1\b",
        r"\1",
    ...
    # Repeated 2-6 word phrase: "a prior a prior", "the previous reporting period the previous reporting period"
    _DUP_PHRASE = re.compile(
        r"\b((?:\w+\s+){1,6}\w+)\s+\1\b", re.IGNORECASE
    )
    prev = None
    while prev != result:
        prev = result
        result = _DUP_PHRASE.sub(r"\1", result)
...
# ---------------------------------------------------------------------------
# Cross-section consistency helpers (called once per PSUR after all sections
# are generated). Each function takes the assembled PSUR dict plus relevant
# deterministic facts and edits in-place. They never call the LLM.
# ---------------------------------------------------------------------------

_FIRST_PSUR_PHRASES = [
    re.compile(r"\bthis is the first PSUR\b[^.]*\.", re.IGNORECASE),
    re.compile(r"\bfirst PSUR (?:for|cycle)\b[^.]*\.", re.IGNORECASE),
    re.compile(r"\bno previous PSUR (?:has been|was) (?:conducted|prepared|issued)\b[^.]*\.", re.IGNORECASE),
    re.compile(r"\bno comparable historical data\b[^.]*\.", re.IGNORECASE),
]

_PREVIOUS_PSUR_PHRASES = [
    re.compile(r"\bthe previous PSUR\b[^.]*\.", re.IGNORECASE),
    re.compile(r"\bprior PSUR cycle\b[^.]*\.", re.IGNORECASE),
    re.compile(r"\bin the (?:previous|prior) reporting period\b[^.]*\.", re.IGNORECASE),
]


def _scrub_strings(obj, patterns, replacement=""):
    if isinstance(obj, str):
        out = obj
        for pat in patterns:
        ...
        return out
    if isinstance(obj, dict):
        return {k: _scrub_strings(v, patterns, replacement) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_scrub_strings(v, patterns, replacement) for v in obj]
    return obj


def enforce_first_psur_consistency(psur, has_previous_psur):
    """Reconcile Section A 'first PSUR' wording with the presence/absence of
    a parsed previous-PSUR input. When the input is missing, prior-period
    claims are scrubbed and the narrative is replaced with the canonical
    first-PSUR statement. When the input is present, 'first PSUR' claims
    are stripped so they can be regenerated against real data.
    """
    if not isinstance(psur, dict):
        return psur
    sections = psur.get("sections", {})
    sec_a = sections.get("A_executive_summary")
    if not isinstance(sec_a, dict):
        return psur

    prev = sec_a.get("previous_psur_actions_status")
    nb = sec_a.get("notified_body_review_status")

    if has_previous_psur:
        # Real prior PSUR exists -> never claim "first PSUR".
        if isinstance(nb, dict):
            for k in ("notified_body_actions_taken", "status_of_nb_actions"):
                v = nb.get(k)
                if isinstance(v, str):
                    nb[k] = _scrub_strings(v, _FIRST_PSUR_PHRASES).strip() or "Not applicable."
        if isinstance(prev, dict):
            for k in ("actions_and_status_from_previous_report",):
                v = prev.get(k)
                if isinstance(v, str):
                    prev[k] = _scrub_strings(v, _FIRST_PSUR_PHRASES)
    else:
        # No prior PSUR -> scrub any invented prior actions and assert "first PSUR".
        if isinstance(prev, dict):
            prev["actions_and_status_from_previous_report"] = (
                "This is the first PSUR for the device; no actions from a previous "
                "PSUR are applicable."
            )
            status_obj = prev.get("status_of_previous_actions")
            if isinstance(status_obj, dict):
                status_obj["status"] = "NOT_APPLICABLE"
                status_obj["details_if_needed"] = None
            else:
                prev["status_of_previous_actions"] = {
                    "status": "NOT_APPLICABLE", "details_if_needed": None,
                }
        if isinstance(nb, dict):
            nb["previous_psur_reviewed_by_notified_body"] = "N_A"
            nb["notified_body_actions_taken"] = (
                "This is the first PSUR for the device. No previous Notified Body "
                "review has been conducted."
            )
            nb["status_of_nb_actions"] = "Not applicable - first PSUR cycle."

        # Also scrub stray "previous PSUR" references inside the executive summary.
        br = sec_a.get("benefit_risk_assessment_conclusion")
        if isinstance(br, dict):
            txt = br.get("high_level_summary_if_adversely_impacted")
            if isinstance(txt, str):
                br["high_level_summary_if_adversely_impacted"] = _scrub_strings(
                    txt, _PREVIOUS_PSUR_PHRASES
                )
    return psur


_FSCA_CLAIM_RE = re.compile(
    r"\b(?:\w+\s+)?(?:Field Safety Corrective Actions?|FSCAs?)\s+"
    r"(?:were\s+(?:initiated|opened|issued|completed|conducted)|"
    r"have been\s+(?:initiated|opened|issued|completed))\b[^.]*\.",
    re.IGNORECASE,
)
_CAPA_CLAIM_RE = re.compile(
    r"\b(?:\w+\s+)?CAPAs?\s+"
    r"(?:were\s+(?:initiated|opened|issued|completed|conducted)|"
    r"have been\s+(?:initiated|opened|issued|completed))\b[^.]*\.",
    re.IGNORECASE,
)
_NO_CAPA_CLAIM_RE = re.compile(
    r"\bno\s+(?:new\s+)?CAPAs?\s+(?:were\s+)?(?:initiated|opened|issued|conducted|identified|reported)\b[^.]*\.",
    re.IGNORECASE,
)


def align_fsca_capa_narrative(psur, fsca_count, capa_count):
    """Reconcile narrative FSCA/CAPA claims across sections D/H/I/A/M with
    the deterministic counts derived from the parsed inputs.
    - If sca_count == 0, strip any sentence that claims N FSCAs were
      initiated/completed during the period from sections A, D, M.
    - Same for CAPAs in sections A, D, I, M.
    """
    if not isinstance(psur, dict):
        return psur
    sections = psur.get("sections", {})
    ...
    for sk in target_keys:
        ...
        if not isinstance(sec, dict):
            continue
        # Iterate string fields only -- preserve table integrity.
        for k, v in list(sec.items()):
            if k.startswith("table_") or k.startswith("_"):
                continue
            if isinstance(v, str):
                new_v = v
                if fsca_count == 0:
                    new_v = _FSCA_CLAIM_RE.sub("", new_v)
                if capa_count == 0:
                    new_v = _CAPA_CLAIM_RE.sub("", new_v)
                elif capa_count > 0:
                    new_v = _NO_CAPA_CLAIM_RE.sub(
                        f"{capa_count} CAPA record(s) were identified during the reporting period. ",
                        new_v,
                    )
                new_v = re.sub(r"\s{2,}", " ", new_v).strip()
                if new_v != v:
                    sec[k] = new_v
            elif isinstance(v, dict):
                for kk, vv in list(v.items()):
                    if isinstance(vv, str):
                        new_vv = vv
                        if fsca_count == 0:
                            new_vv = _FSCA_CLAIM_RE.sub("", new_vv)
                        if capa_count == 0:
                            new_vv = _CAPA_CLAIM_RE.sub("", new_vv)
                        elif capa_count > 0:
                            new_vv = _NO_CAPA_CLAIM_RE.sub(
                                f"{capa_count} CAPA record(s) were identified during the reporting period. ",
                                new_vv,
                            )
                        new_vv = re.sub(r"\s{2,}", " ", new_vv).strip()
                        if new_vv != vv:
                            v[kk] = new_vv
    return psur


_ALL_WITHIN_THRESHOLD_RE = re.compile(
    r"\ball\s+(?:complaint\s+)?(?:categories|rates)\s+(?:fell|were|remained)\s+"
    r"within\s+(?:established\s+)?(?:Upper Control Limits?|expected thresholds?|thresholds?)\b[^.]*\.",
    re.IGNORECASE,
)
_NO_EXCEEDANCE_RE = re.compile(
    r"\bno\s+(?:complaint\s+)?(?:categories|rates)\s+exceeded\b[^.]*\.",
    re.IGNORECASE,
)


def align_threshold_claims(psur, any_threshold_exceeded):
    """Strip blanket 'all rates within threshold' claims when at least one
    RACT row has been flagged as exceeding its maximum expected rate."""
    if not isinstance(psur, dict) or not any_threshold_exceeded:
        return psur
    sections = psur.get("sections", {})
    for sk in ("A_executive_summary", "F_product_complaint_types_counts_and_rates",
               "G_information_from_trend_reporting", "M_findings_and_conclusions"):
        sec = sections.get(sk)
        if not isinstance(sec, dict):
            continue
        ...
        def _walk(o):
            if isinstance(o, dict):
                for k, vv in list(o.items()):
                    if isinstance(vv, str):
                        new = _ALL_WITHIN_THRESHOLD_RE.sub("", vv)
                        new = _NO_EXCEEDANCE_RE.sub("", new)
