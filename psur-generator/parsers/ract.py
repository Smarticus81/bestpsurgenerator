...
def _parse_ract_json(filepath: Path) -> Dict[str, Any]:
    """Parse a RACT JSON file (native structured format)."""
    ...
    for h in raw_hazards:
        if not isinstance(h, dict):
            continue
        hazard_id = str(h.get("hazard_id") or h.get("id") or "").strip()
        name = str(h.get("name") or h.get("hazard") or "").strip()
        rate = h.get("max_rate_per_1000")
        if rate is None:
            rate = h.get("max_expected_rate") or h.get("max_expected_rate_per_1000")
        try:
        ...
        hazards.append({
            ...
            "max_expected_rate": rate_f,
            "imdrf_code": h.get("imdrf_code") or "",
    ...
    result = {
        ...
        "max_expected_rates": max_rates,
        "risk_summary": risk_summary,
        "global_thresholds": data.get("global_thresholds") or {},
        "total_hazards": len(hazards),
    }
    logger.info(
