...
_LARGE_COUNT = re.compile(r"\b\d{3,}\b")  # counts >= 100 — small ints are too noisy
...
class TraceabilityChecksMixin:
    ...
    def _scan_sentence(
        ...
        for m in _LARGE_COUNT.findall(residual):
            status = "ok" if m in registry["counts"] else "unsourced_count"
            findings.append({
                "type": "count",
                "kind": "int",
                "value": m,
    ...
    def _check_traceability(
        ...
        # Resolve reporting window from cover page if not supplied
        if reporting_start is None or reporting_end is None:
            cover = psur.get("psur_cover_page", {}) or {}
            for k in ("reporting_period_start", "period_start", "start_date"):
                v = cover.get(k)
                if v:
                    try:
                        reporting_start = datetime.fromisoformat(str(v)[:10]).date()
                        break
                    except ValueError:
                        pass
            for k in ("reporting_period_end", "period_end", "end_date"):
                v = cover.get(k)
                if v:
                    try:
                        reporting_end = datetime.fromisoformat(str(v)[:10]).date()
                        break
                    except ValueError:
                        pass
