...
def _parse_fsca_csv(path: Path) -> Dict[str, Any]:
    """Parse a simple FSCA CSV into structured rows for Section H/Table 8."""
...
def parse_all_inputs(
    *,
    ...
    # ── Knowledge-layer skill invocation (deterministic pre-processing) ──
    skill_invocations: List[Dict[str, Any]] = []
    try:
        from knowledge import get_skill_registry, SkillContext
        ctx = SkillContext(data={
            "sales_data": parsed_data.get("sales") or {},
            "complaints": (parsed_data.get("complaints") or {}).get("summaries", []),
            "reporting_period_start": parsed_data.get("_reporting_period_start"),
            "reporting_period_end": parsed_data.get("_reporting_period_end"),
        })
        sk_reg = get_skill_registry()
        # Sales aggregate is already partly handled in statistics.py; we
        # still invoke it here so its provenance is recorded.
        sk_reg.invoke("psur-sales-aggregate", ctx)
        sk_reg.invoke("psur-imdrf-classify", ctx)
        # Persist enriched complaints back to parsed_data
        if ctx.get("complaints"):
            try:
                parsed_data.setdefault("complaints", {})["summaries"] = ctx.get("complaints")
            except Exception:
                pass
        if ctx.get("sales_periods"):
            parsed_data.setdefault("sales", {})["_skill_periods"] = ctx.get("sales_periods")
        skill_invocations = ctx.invocations
    except Exception as exc:
        logger.warning("Knowledge skill invocation failed: %s", exc)
