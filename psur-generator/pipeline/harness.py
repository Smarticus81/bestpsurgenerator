...
def run_harness(
    *,
    ...
    reconcile_skill_narratives(psur, context, stats)
...
def inject_skill_authoritative_tables(
    ...
    sec_d["table_2_serious_incidents_by_imdrf_annex_a_by_region"] = [{
        "region": "EEA+TR+XI / UK",
        "imdrf_code_and_term": "N/A - no EU/UK serious incidents meeting the reporting threshold"
            if ww_count == 0 else "See source vigilance records",
        "count": ww_count,
        "rate_percent": 0.0 if ww_count == 0 else None,
        "complaint_number": "N/A" if ww_count == 0 else "See source vigilance records",
    }]
    sec_d["table_3_serious_incidents_by_imdrf_annex_c_investigation_findings_by_region"] = [{
        "region": "EEA+TR+XI / UK",
        "imdrf_code_and_term": "N/A - no EU/UK serious incidents meeting the reporting threshold"
            if ww_count == 0 else "See source vigilance records",
        "count": ww_count,
    ...
    sec_d["table_4_health_impact_by_investigation_conclusion"] = [{
        "health_impact": "N/A - no EU/UK serious incidents meeting the reporting threshold"
            if ww_count == 0 else "See source vigilance records",
        "count": ww_count,
        "conclusion_1_pct": 0.0 if ww_count == 0 else None,
        "conclusion_2_pct": 0.0 if ww_count == 0 else None,
        "conclusion_3_pct": 0.0 if ww_count == 0 else None,
        "conclusion_4_pct": 0.0 if ww_count == 0 else None,
    }]
...
def _count_records(value: Any) -> int:
    ...
    if isinstance(value, dict):
        for key in ("records", "capa_summaries", "fsca_summaries", "items"):
...
def _record_status(record: Dict[str, Any]) -> str:
    return str(record.get("status") or record.get("state") or "").strip().lower()


def _any_open(records: List[Dict[str, Any]]) -> bool:
    open_terms = ("open", "progress", "pending", "ongoing", "active")
    return any(any(term in _record_status(r) for term in open_terms) for r in records)


def _rate_pct(count: int, denominator: int) -> float:
    return round((count / denominator) * 100, 4) if denominator else 0.0


def _display_pct(count: int, denominator: int) -> str:
    return f"{_rate_pct(count, denominator):.4f}%"


def _period_label(context: HarnessContext) -> str:
    ri = context.get("report_identity") or {}
    start = ri.get("reporting_period_start") or ""
    end = ri.get("reporting_period_end") or ""
    return f"{start} to {end}" if start and end else "the reporting period"


def reconcile_skill_narratives(
    psur: Dict[str, Any],
    context: HarnessContext,
    stats: PSURStatistics,
) -> None:
    """Rewrite high-risk narrative fields from deterministic source data.

    The LLM writes useful prose, but auditors read contradictions between prose,
    checkboxes, and tables as data-integrity failures. This pass keeps the
    renderer-facing text aligned with the harness facts immediately before the
    DOCX is generated.
    """
    sections = psur.setdefault("sections", {})
    period = _period_label(context)
    device = (context.get("device_scope") or {}).get("device_trade_names") or []
    device_name = device[0] if device else "the device"

    cd = context.get("complaint_data") or {}
    vd = context.get("vigilance_data") or {}
    sd = context.get("sales_data") or {}
    capa_data = context.get("capa_data") or {}
    ed = context.get("external_databases") or {}
    pd_ = context.get("pmcf_data") or {}
    bd = context.get("benefit_risk_determination") or {}
    actions = context.get("actions_and_updates") or {}
    ri = context.get("report_identity") or {}
    raw = context.get("raw_inputs") or {}

    denominator = int(stats.total_units_sold or sd.get("worldwide_current_total") or 0)
    complaint_count = int(stats.total_complaints or cd.get("total_complaints") or 0)
    complaint_rate = _display_pct(complaint_count, denominator)
    fda_count = int((vd.get("mdr_reports_us_fda") or {}).get("total_count") or 0)
    sieu = vd.get("serious_incidents_eu_uk") or {}
    eu_uk_serious = int(sieu.get("count_worldwide_eu_mdr_threshold") or 0)
    fsca_records = (vd.get("fsca") or {}).get("records") or []
    capa_records = capa_data.get("records") or []
    if not isinstance(fsca_records, list):
        fsca_records = []
    if not isinstance(capa_records, list):
        capa_records = []
    fsca_count = len(fsca_records)
    capa_count = len(capa_records)

    is_first = bool(ri.get("is_first_psur"))
    previous_available = bool(raw.get("previous_stats")) or bool(ri.get("preceding_period_end"))

    sec_a = sections.setdefault("A_executive_summary", {})
    prev_block = sec_a.setdefault("previous_psur_actions_status", {})
    if is_first and not previous_available:
        prev_block["actions_and_status_from_previous_report"] = (
            f"This is the first PSUR for {device_name}. No previous PSUR actions "
            "were available for follow-up during this reporting period."
        )
        prev_block["status_of_previous_actions"] = {
            "status": "NOT_APPLICABLE",
            "details_if_needed": "No previous PSUR actions exist for the first PSUR cycle.",
        }
        nb = sec_a.setdefault("notified_body_review_status", {})
        nb["previous_psur_reviewed_by_notified_body"] = "N_A"
        nb["notified_body_actions_taken"] = "Not applicable - first PSUR cycle."
        nb["status_of_nb_actions"] = "Not applicable - first PSUR cycle."
    else:
        prior_status = "IN_PROGRESS" if _any_open(capa_records + fsca_records) else (
            "COMPLETED" if (capa_records or fsca_records) else "NOT_APPLICABLE"
        )
        detail = (
            f"{capa_count} CAPA record(s) and {fsca_count} FSCA record(s) are "
            f"tracked in the current PSUR data set."
        )
        prev_block["actions_and_status_from_previous_report"] = (
            f"This PSUR is a subsequent report for {device_name}; previous-period "
            f"data are available for comparison. {detail} "
            f"The previous-action status is {prior_status.replace('_', ' ').title()} "
            "based on the source action records available to the harness."
        )
        prev_block["status_of_previous_actions"] = {
            "status": prior_status,
            "details_if_needed": detail,
        }

    a_br = sec_a.setdefault("benefit_risk_assessment_conclusion", {})
    harness_conclusion = str(bd.get("conclusion") or "").upper()
    if "ADVERSELY_IMPACTED" in harness_conclusion and "NOT" not in harness_conclusion:
        a_br["conclusion"] = "ADVERSELY_IMPACTED"
    elif harness_conclusion:
        a_br["conclusion"] = "NOT_ADVERSELY_IMPACTED_UNCHANGED"
    a_br["high_level_summary_if_adversely_impacted"] = (
        f"This PSUR covers {device_name} for {period}. During the reporting period, "
        f"{denominator:,} units were distributed and {complaint_count} complaints "
        f"were received, corresponding to an overall complaint rate of {complaint_rate}. "
        f"{fda_count} complaint(s) met the US FDA MDR reporting threshold. "
        f"{eu_uk_serious} complaint(s) met the EU/UK serious incident threshold used "
        f"for the serious-incident tables. {fsca_count} FSCA record(s) and "
        f"{capa_count} CAPA record(s) were identified in the source data for this "
        "reporting period. The benefit-risk conclusion is aligned with Section M."
    )

    sec_d = sections.setdefault("D_information_on_serious_incidents", {})
    sec_d["narrative_summary"] = (
        f"During {period}, {fda_count} complaint(s) met the US FDA MDR reporting "
        "threshold and are discussed as FDA MDR reports. These reports are not "
        "automatically equivalent to EU/UK serious incidents. Based on the harness "
        f"EU/UK threshold classification, {eu_uk_serious} complaint(s) met the "
        "EU/UK serious incident threshold for inclusion in Tables 2-4. "
        f"Accordingly, the serious-incident tables below show {eu_uk_serious} "
        "EU/UK serious incident(s), while FDA MDR reports remain narrative context. "
        "No deaths were identified in the deterministic source data."
    )
    sec_d["new_incident_types_identified_this_cycle"] = (
        "No new incident types were identified by the deterministic source-data "
        "review. Any FDA MDR reports are evaluated through complaint trending, CAPA, "
        "FSCA, and benefit-risk review without being counted as EU/UK serious "
        "incidents unless they meet the applicable threshold."
    )

    sec_f = sections.setdefault("F_product_complaint_types_counts_and_rates", {})
    harms = stats.complaints_by_harm or {}
    harm_sentence = ", ".join(
        f"{count} {harm}" for harm, count in sorted(harms.items(), key=lambda item: (-item[1], item[0]))
    ) or "no harm categories were available"
    f_calc = sec_f.setdefault("complaint_rate_calculation", {})
    f_calc["method_description_and_justification"] = (
        f"The complaint rate is calculated as complaints divided by units distributed "
        f"during the same reporting period. For {period}, the calculation is "
        f"{complaint_count} complaints / {denominator:,} units x 100 = {complaint_rate}. "
        "The same denominator is used consistently across complaint-rate and trend analyses."
    )
    f_table = sec_f.setdefault(
        "annual_number_of_complaints_and_complaint_rate_by_harm_and_medical_device_problem", {}
    )
    f_table["commentary_context_for_exceedances"] = (
        f"Table 7 reconciles to {complaint_count} total complaint(s). The deterministic "
        f"harm distribution is: {harm_sentence}. These counts are the controlling "
        "source for Section F narrative and must match the Table 7 grand total."
    )

    sec_c = sections.setdefault("C_volume_of_sales_and_population_exposure", {})
    region_rows = list(stats.section_c_region_rows or [])
    current_rows = [r for r in region_rows if r.get("region") != "Worldwide"]
    nonzero_rows = [
        f"{r.get('region')}: {int(r.get('units') or 0):,}"
        for r in current_rows
        if int(r.get("units") or 0) > 0
    ]
    regional_text = "; ".join(nonzero_rows) if nonzero_rows else "no regional sales rows with non-zero units"
    preceding_totals = [
        int((next((r for r in region_rows if r.get("region") == "Worldwide"), {}) or {}).get(k) or 0)
        for k in ("units_p1", "units_p2", "units_p3")
    ]
    if any(preceding_totals):
        prior_text = (
            "Preceding-period sales data were available in the deterministic "
            f"statistics payload: {', '.join(f'{v:,}' for v in preceding_totals if v)}."
        )
    else:
        prior_text = "Preceding-period regional sales data were not available in the parsed source data."
    sec_c.setdefault("sales_data_analysis", {})["narrative_analysis"] = (
        f"During {period}, {denominator:,} units were distributed worldwide. "
        f"The current-period regional distribution from the statistics engine is: {regional_text}. "
        f"{prior_text} Section C narrative is limited to these deterministic table values."
    )
    if region_rows:
        sec_c["table_1_sales_by_region"] = {
            "use_if_psur_frequency": "ANNUALLY",
            "annual_format": {
                "date_ranges": list(stats.section_c_period_labels or []),
                "rows": [
                    {
                        "region": r.get("region", ""),
                        ...
                        "current_data_collection_period": int(r.get("units") or 0),
                        "percent_of_global_sales": r.get("pct_current") or (
                            round((int(r.get("units") or 0) / denominator) * 100, 1)
    ...
    previous_units = int((raw.get("previous_stats") or {}).get("total_units_sold") or 0)
    previous_complaints = int((raw.get("previous_stats") or {}).get("total_complaints") or 0)
    g_overall = sec_g.setdefault("overall_monthly_complaint_rate_trending", {})
    if previous_units or previous_complaints:
        prev_text = (
            f"Previous-period comparator data were available: {previous_complaints} "
            f"complaint(s) across {previous_units:,} unit(s)."
        )
    else:
        prev_text = (
            "No previous-period comparator totals were available in the parsed source data."
        )
    g_overall["breaches_commentary_and_actions"] = (
        f"Trend analysis used the monthly complaint counts and monthly unit denominator "
        f"from the reporting period. {prev_text} The current reporting period had "
        f"{complaint_count} complaint(s) across {denominator:,} unit(s). Any trend "
        "conclusion is based on these deterministic totals and the UCL analysis "
    ...
    if fsca_count:
        sec_h["summary_or_na_statement"] = (
            f"{fsca_count} FSCA record(s) were identified in the source data for {period}. "
            "Section H therefore reports the FSCA records in Table 8; the narrative must "
            "not state that no FSCAs occurred."
        )
        sec_h["table_8_fsca_initiated_current_period_and_open_fscas"] = [
            {
                "type_of_action": r.get("type_of_action") or r.get("type") or "FSCA",
                "manufacturer_reference_number": (
                    r.get("manufacturer_reference_number")
                    or r.get("reference_number")
                    or r.get("fsca_id")
                    or r.get("id")
                    or "N/A"
                ),
                "issuing_date": r.get("issuing_date") or r.get("date") or "N/A",
                "scope": r.get("scope") or "See FSCA source record",
                "status": r.get("status") or "N/A",
                "rationale_and_description": (
                    r.get("rationale_and_description")
                    or r.get("description")
                    or "See FSCA source record"
                ),
                "impacted_regions": r.get("impacted_regions") or r.get("region") or "N/A",
                "date_reported_to_mhra": r.get("date_reported_to_mhra") or "N/A",
            }
            for r in fsca_records
        ]
    else:
        sec_h["summary_or_na_statement"] = (
            f"No FSCA records were provided for {period}. Table 8 is therefore marked "
    ...
    if capa_count:
        sec_i["summary_or_na_statement"] = (
            f"{capa_count} CAPA record(s) were identified in the source data for {period}. "
            "Section I therefore reports those CAPA records in Table 9; the narrative "
            "must not state that no CAPAs were initiated or open."
        )
        sec_i["table_9_capa_initiated_current_reporting_period"] = [
            {
                "capa_number": r.get("capa_number") or r.get("number") or r.get("id") or "N/A",
                "initiation_date": r.get("initiation_date") or r.get("date_opened") or "N/A",
                "scope": r.get("scope") or "See CAPA source record",
                "status": r.get("status") or "N/A",
                "description": r.get("description") or "See CAPA source record",
        ...
        sec_i["summary_or_na_statement"] = (
            f"No CAPA records were provided for {period}. Table 9 is therefore marked "
            "not applicable for this reporting period."
        )
        sec_i["table_9_capa_initiated_current_reporting_period"] = [{
            "capa_number": "N/A",
    ...
    db_rows = ed.get("databases_reviewed") or sec_k.get("table_10_adverse_events_and_recalls") or []
    db_total = sum(int(r.get("total_matches", 0) or 0) for r in db_rows if isinstance(r, dict))
    sec_k["registries_reviewed_summary"] = (
        f"External database review is summarized in Table 10. The table contains "
        f"{len(db_rows)} reviewed database row(s) and {db_total} total match(es) "
        "as recorded in the source table. The narrative conclusion is limited to "
        "the table contents and does not add events that are absent from Table 10."
    )

    sec_j = sections.setdefault("J_scientific_literature_review", {})
    lr = context.get("literature_review") or {}
    lit_method = lr.get("search_methodology_summary") or ""
    if not lit_method:
        sec_j["literature_search_methodology"] = (
            f"No formal literature search report was provided in the parsed source "
            f"package for {period}. Section J is therefore limited to noting the "
            "absence of source literature results."
        )
        sec_j["number_of_relevant_articles_identified"] = 0
        sec_j["summary_of_new_data_performance_or_safety"] = (
            "No source literature results were available for deterministic summary. "
            "No literature-derived safety or performance conclusions are made in this PSUR."
        )
        sec_j["comparison_with_similar_devices"] = (
            "No sourced comparator literature data were provided; no benchmark rates "
            "or competitor comparisons are stated."
        )
        sec_j["previously_unassessed_risks"] = (
            "No literature-derived previously unassessed risks were identified because "
            "no literature result set was provided."
        )
        sec_j["state_of_the_art_changes"] = (
            "No state-of-the-art changes were identified from literature because no "
            "source literature result set was provided."
        )

    sec_l = sections.setdefault("L_pmcf", {})
    pmcf_plan = pd_.get("pmcf_plan_number") or "N/A"
    sec_l["summary_or_na_statement"] = (
        f"No formal PMCF evaluation report results were provided for {period}. "
        f"The PMCF plan reference is {pmcf_plan}. The benefit-risk conclusion does "
        "not rely on unprovided PMCF outcome data; it relies on complaint, vigilance, "
        "trend, CAPA, FSCA, literature, and external database evidence available in "
        "the source package."
    )

    sec_m = sections.setdefault("M_findings_and_conclusions", {})
    conclusion = bd.get("conclusion") or "NOT_ADVERSELY_IMPACTED"
    conclusion_text = "not adversely impacted" if "NOT" in conclusion else "adversely impacted"
    sec_m["benefit_risk_profile_conclusion"] = (
        f"Based on deterministic review of {denominator:,} units distributed, "
        f"{complaint_count} complaint(s), {fda_count} FDA MDR report(s), "
        f"{eu_uk_serious} EU/UK serious incident(s), {fsca_count} FSCA record(s), "
        f"and {capa_count} CAPA record(s), the benefit-risk profile is {conclusion_text}. "
        "This conclusion is tied to the source-data counts used in Sections C, D, F, H, "
        "and I, and does not rely on contradictory or unverified action statements."
    )
    action_details = (
        f"Actions and updates were determined from source records: CAPA initiated = "
        f"{bool(actions.get('capa_initiated'))}; FSCA initiated = "
        f"{bool(actions.get('fsca_initiated'))}; RMF updated = "
        f"{bool(actions.get('rmf_updated'))}; CER updated = {bool(actions.get('cer_updated'))}."
    )
    sec_m.setdefault("actions_taken_or_planned", {})["action_details_and_follow_up"] = action_details
...
def sanitize_skill_render_content(psur: Dict[str, Any], context: HarnessContext) -> None:
    ...
    def _clean(value: Any) -> Any:
        if isinstance(value, str):
            ...
            result = result.replace("section we", "Section I")
            result = re.sub(r"\ba prior\s+a prior\s+FSCA\b", "prior FSCA", result, flags=re.IGNORECASE)
            result = re.sub(
                r"\ba\s+a\s+CAPA from the previous reporting period\s+the previous reporting period",
                "CAPA from the previous reporting period",
                result,
                flags=re.IGNORECASE,
            )
            result = re.sub(
    ...
    for section in (psur.get("sections") or {}).values():
